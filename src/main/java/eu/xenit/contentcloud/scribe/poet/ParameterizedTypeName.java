...
public final class ParameterizedTypeName extends TypeName {
  ...
  private ParameterizedTypeName(ParameterizedTypeName enclosingType, ClassName rawType,
    ...
    super(annotations);
    this.rawType = Util.checkNotNull(rawType, "rawType == null").annotated(annotations);
    this.enclosingType = enclosingType;
    this.typeArguments = Util.immutableList(typeArguments);

    Util.checkArgument(!this.typeArguments.isEmpty() || enclosingType != null,
        "no type arguments: %s", rawType);
    for (TypeName typeArgument : this.typeArguments) {
      Util.checkArgument(!typeArgument.isPrimitive() && typeArgument != VOID,
