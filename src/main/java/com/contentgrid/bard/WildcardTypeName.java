...
public final class WildcardTypeName extends TypeName {
  ...
  @Override CodeWriter emit(CodeWriter out) throws IOException {
    ...
    return upperBounds.get(0).equals(OBJECT)
        ? out.emit("?")
  ...
  public static WildcardTypeName subtypeOf(Type upperBound) {
    return subtypeOf(get(upperBound));
  }
  ...
  public static WildcardTypeName supertypeOf(Type lowerBound) {
    return supertypeOf(get(lowerBound));
  }
  ...
  static TypeName get(
    ...
    if (extendsBound == null) {
      ...
      if (superBound == null) {
        ...
        return supertypeOf(get(superBound, typeVariables));
      }
    } else {
      return subtypeOf(get(extendsBound, typeVariables));
    }
  }
